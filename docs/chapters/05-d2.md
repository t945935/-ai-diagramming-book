...
[draw.io 樣章](08-drawio.md) 用可編輯節點及連線交付架構。本章保留相同的業務元件與呼叫關係，改用 D2 原始碼，讓讀者比較「改文字再生成」與「直接拖拉圖形」的差異。
...
[完整驗證紀錄](../../examples/d2/order-platform/verification.md) 區分已執行與未測項目。首次製作時未做跨平台或 CI 測試；現行核心 CI 已檢查 D2／draw.io 的指定呼叫端點，但不執行 D2 渲染，詳見 [驗證狀態導讀](../validation-status.md)。
...
當團隊主要以文字、Git 和生成流程維護圖時，D2 很合適。當修改主要發生在會議中、需要逐個物件拖拉與精確對齊時，可參考 draw.io 樣章。先決定誰接手維護，再選工具。
...
[全書目錄](../chapters.md) · [共用術語](../glossary.md) · [驗證狀態](../validation-status.md) · [上一章](04-mermaid.md) · [下一章](06-plantuml.md)
